...
from game_core.agent import Agent
from game_core.agent_handler import AgentHandler
...
for item in pkgutil.iter_modules(agents_pkg.__path__):
    package_name = f"{agents_pkg.__name__}.{item.name}"
...
if not agent_classes:
    raise RuntimeError(
        "No Agent classes found. Put your agent .py files in game_core/agents/ and make sure each class subclasses Agent."
    )
